...
def test_registered_launcher_works_after_installing_elsewhere(tmp_path):
    """Launch the installed config, including paths with spaces and Unicode."""
    installed = tmp_path / "plugin cache 資料" / "freeup-space"
    shutil.copytree(ROOT, installed, ignore=shutil.ignore_patterns(
        "__pycache__", ".pytest_cache", "*.egg-info", "build", "dist", ".freeup-space"))
    widget = "<!doctype html><title>Relocated widget fixture</title>"
    (installed / "assets/widget.html").write_text(widget, encoding="utf-8")
    workspace = tmp_path / "unrelated workspace"
    workspace.mkdir()
    config = json.loads((installed / ".mcp.json").read_text())["mcpServers"]["freeup-space"]
    # Codex resolves a relative plugin MCP cwd against the installed plugin root.
    configured_cwd = config.get("cwd")
    cwd = installed / configured_cwd if configured_cwd else workspace
    messages = [
    ...
    result = subprocess.run([config["command"], *config.get("args", [])],
                            cwd=cwd, env={**os.environ, **config.get("env", {})},
                            input="\n".join(json.dumps(m) for m in messages) + "\n",
                            text=True, encoding="utf-8", capture_output=True, timeout=20)
    assert result.returncode == 0, result.stderr
    responses = [json.loads(line)["result"] for line in result.stdout.splitlines()]
    assert any(t["name"] == "open_widget" for t in responses[1]["tools"])
    assert responses[2]["contents"][0]["text"] == widget
    assert responses[3]["_meta"]["widget"]["status"] == "choose-mode"
    assert not list(workspace.iterdir()), "Startup must not create a cleanup run"
